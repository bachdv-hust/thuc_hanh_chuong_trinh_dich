...
Token *readNumber(void)
{
  ...
  while (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD){
    ...
    // nhieu hon 1 dau cham thi show loi
    if (check_float > 1){
      error(ERR_INVALID_NUMBER_FLOAT, token->lineNo, token->colNo);
    }
...
Token *getToken(void)
{
  ...
  case CHAR_RPAR:
    ...
    return token;
  default:
    //bang mot cach than ky nao do no se vao day
